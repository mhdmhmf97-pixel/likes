...
def send_like_request(token, TARGET):
    ...
    try:
        ...
        return {"token": token[:20]+"...", "status_code": resp.status_code, "response_text": resp.text}
...
def send_like():
    ...
    while likes_sent < max_likes:
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:  # تقليل الخيوط لتجنب مشاكل
            futures = {executor.submit(send_like_request, token, TARGET): token for token in token_items}
            for future in as_completed(futures):
                token = futures[future]
                res = future.result()
                # أي رد 200 = نجاح، الباقي تجاوز
